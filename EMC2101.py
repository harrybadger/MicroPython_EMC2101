...
EMC2101_EXTERNAL_TEMP_MSB = const(0x01)  # high byte for the external temperature reading
...
EMC2101_TACH_LIMIT_LSB = const(0x48)  # Tach low-speed setting low byte. INVERSE OF THE SPEED
EMC2101_TACH_LIMIT_MSB = const(0x49)  # Tach low-speed setting high byte. INVERSE OF THE SPEED
EMC2101_FAN_CONFIG = const(0x4A)  # General fan config register
EMC2101_FAN_SPINUP = const(0x4B)  # Fan spinup behavior settings
EMC2101_REG_FAN_SETTING = const(0x4C)  # Fan speed for non-LUT settings, as a % PWM duty cycle
EMC2101_PWM_FREQ = const(0x4D)  # PWM frequency setting
EMC2101_PWM_DIV = const(0x4E)  # PWM frequency divisor
EMC2101_LUT_HYSTERESIS = const(0x4F)  # The hysteresis value for LUT lookups when temp is decreasing
